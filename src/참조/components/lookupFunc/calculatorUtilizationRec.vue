  ...
  <div class="report_wrap">
    ...
    <div class="list-items">
      <ul class="list-title">
        ...
        <li class="location">
          <span
            >장소<button @click.prevent="resident_fee_click">
              <font-awesome-icon
                icon="caret-down"
        ...
        <li>
          <span
            >정산기ID<button @click.prevent="resident_fee_click">
              <font-awesome-icon
                icon="caret-down"
                v-if="sort_item.resident_fee_click"
              />
        ...
        <li>
          <span
            >결제요금
            <button @click.prevent="paid_fee_click">
              <font-awesome-icon icon="caret-down" v-if="sort_item.paid_fee" />
              <font-awesome-icon icon="caret-up" v-else /></button
          ></span>
        </li>
        <li>
      ...
      <ul class="list-wrap">
        ...
        >
          ...
          /></span>
          <span class="groupName"
            ><input type="text" v-model="res.lp" readonly
          /></span>
          <span class="groupName"
            ><input type="text" v-model="res.parking_fee" readonly
          /></span>
          <span class="groupName"
            ><input type="text" v-model="res.machineid" readonly
          /></span>
          <span class="groupName"
            ><input type="text" v-model="res.paid_fee" readonly
          /></span>
          <span class="groupName"
            ><input type="text" v-model="res.how_to_pay" readonly
          /></span>
          <span class="contents"
            ><input
              type="text"
  ...
  methods: {
    changeTitle() {
      this.mainTitle.forEach(title => {
        title.subMenu.forEach(add => {
          if (add.add === "calculatorUtilizationRec") this.titles = add.title
        })
      })
    },
    modifyClose() {
    ...
    socket_process(data) {
      if (data.status === "ok" && data.kind === "get_payment_list") {
        ...
        //  if (data.status === "ok" && data.kind ===  "payment_result_list") {
        ...
        for (let i = 0; i < vehicle_obj_list.length; i++) {
          ...
          if (obj.obj_in) {
            ...
            if (obj2.payment_list && obj2.pre_payment_list) {
              console.log(
                "payment_list : ",
                obj2.payment_list,
              ...
              for (let k = 0; k < obj2.payment_list.length; k++) {
                obj1.location = "출구"
                obj1.paid_fee += obj2.payment_list[k].paid_fee
                if (obj2.payment_list[k].mid) {
                  obj1.machineid = obj2.payment_list[k].mid
                } else {
                  obj1.machineid = obj2.payment_list[k].machineid
                }
                obj1.time =
                  obj2.payment_list[k].paydate +
                  "_" +
                  obj2.payment_list[k].paytime
                obj1.how_to_pay = obj2.payment_list[k].paytype
              }
              console.log(
                "pre_payment_list : ",
                obj2.pre_payment_list,
                obj2.pre_payment_list[0].paid_fee
              )
              obj1.pre_payment_list = []
              //  obj1.payment_list = obj2.payment_list
              for (let k = 0; k < obj2.pre_payment_list.length; k++) {
                obj1.location = "출구"
                obj1.paid_fee += obj2.pre_payment_list[k].paid_fee
                if (obj2.pre_payment_list[k].mid) {
                  obj1.machineid += obj2.pre_payment_list[k].mid
                } else {
                  obj1.machineid += obj2.pre_payment_list[k].machineid
                }
                obj1.time =
                  obj2.pre_payment_list[k].paydate +
                  "_" +
                  obj2.pre_payment_list[k].paytime
                obj1.how_to_pay = obj2.pre_payment_list[k].paytype
              }
              this.vehicle_obj_list.push(obj1)
            } else if (obj2.payment_list) {
              console.log(
                "payment_list : ",
                obj2.payment_list,
                obj2.payment_list[0].paid_fee
              ...
              for (let k = 0; k < obj2.pre_payment_list.length; k++) {
                ...
                obj1.time =
                  ...
                  "_" +
                  obj2.pre_payment_list[k].paytime
                if (obj2.pre_payment_list[k].pay_type) {
                  obj1.how_to_pay = obj2.pre_payment_list[k].pay_type
                } else {
                  obj1.how_to_pay = obj2.pre_payment_list[k].paytype
                }
              }
    ...
    download: function() {
      ...
      XLSX.writeFile(
        wb,
        config.parking_name + "_정산기이용기록_" + postfix + ".xlsx"
      )
    }
  },
  created() {
