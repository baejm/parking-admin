    ...
    >
      <div>
        ...
        <div class="form_window">
          <form>
            <dl>
              <!--   <dd class="batch"><label><input   v-model="batch.alert"></label></dd> -->
              <dt>세대통보</dt>
              <dd><input type="text" v-model="batch.alert" required /></dd>
            </dl>
            <div class="btns">
    ...
    <div class="list-items">
      ...
      <ul class="list-wrap">
        ...
        >
          ...
          /></span>
          <span class="name"><input type="text" v-model="res.name"/></span>
          <span class="phone"><input type="text" v-model="res.phone"/></span>
          <span class="dong"><input type="text" v-model="res.dong"/></span>
          <span class="ho"><input type="text" v-model="res.ho"/></span>
          <span class="alert"><input type="text" v-model="res.alert"/></span>
          <span class="contents"
            ><input
              type="text"
...
export default {
  ...
  created() {
    this.$socket.on("message_from_server", data => {
      if (data.kind === "get_vehicle_list_1") {
        this.vehicle_list_1(data)
      } else if (
        data.kind === "create_vehicle_1_list" ||
        data.kind === "update_vehicle_1_list" ||
        data.kind === "delete_vehicle_1_list" ||
        data.kind === "batch_vehicle_list_1" ||
        data.kind === "batch_vehicle_update_list"
      ) {
        this.search()
      }
    })
  ...
  methods: {
    changeTitle() {
      this.freeMainTitles.forEach(title => {
        title.subMenu.forEach(add => {
          // console.log("changeTitle : ", add)
          if (add.add === "vehicleManagerM") {
            this.titles = add.title
            //  console.log("changeTitle : ", this.titles)
          }
        })
      })
    },
    upDown_click: function(e) {
      ...
      if (e.sortBy === false) {
        ...
        e.sortBy = true
        temp10 = _.sortBy(this.list_vehicle_list, e.standard)
        temp10.reverse()
    ...
    registNew(e) {
      ...
      obj.kind = "create_vehicle_1_list"
      console.log("obj :", obj)
      this.sendMessage(obj)
    },
    modifyVehicle() {
      ...
      obj.kind = "update_vehicle_1_list"
      this.sendMessage(obj)
    },
    deleteVehicle() {
      this.modifyClose()
      ...
      obj.time = Date.now()
      obj.number = this.current.lp.replace(/[가-힝]/gi, "")
      obj.kind = "delete_vehicle_1_list"
      this.sendMessage(obj)
    },
    batchModify() {
    ...
    search() {
      var obj = {}
      //  console.log("search : ", this.searchNumber, this.searchDong, this.searchHo)
