      ...
      <div>
        ...
        <div class="form_window">
          <form>
            <dl>
              <dt>그룹명</dt>
              <dd><input type="text" required v-model="m.groupName" /></dd>
            </dl>
            <dl>
              <dt>비고</dt>
    ...
    <div class="list-items">
      ...
      <ul class="list-wrap">
        <li
          ...
          :key="index"
          @click="modifyOpen(group)"
        >
          <span> {{ index + 1 }}</span>
          <span><input type="text" v-model="group.groupName" readonly/></span>
          <span><input type="text" v-model="group.str_time" readonly/></span>
          <span><input type="text" v-model="group.contents"/></span>
        </li>
      </ul>
    </div>
  </div>
...
export default {
  data() {
    return {
      listItem: [
        { title: "번호" },
        { title: "그룹명", sortBy: false, standard: "groupName" },
        { title: "등록일", sortBy: false, standard: "str_time" },
  ...
  methods: {
    upDown_click: function(e) {
      ...
      if (e.sortBy === false) {
        ...
        e.sortBy = true
        temp10 = _.sortBy(this.group_list, e.standard)
        temp10.reverse()
        return (this.group_list = temp10)
      } else {
        e.sortBy = false
        temp10 = _.sortBy(this.group_list, e.standard)
        return (this.group_list = temp10)
    ...
    changeTitle() {
      this.mainTitle.forEach(title => {
        title.subMenu.forEach(add => {
          if (add.add === "groupListM") this.titles = add.title
        })
      })
    },
    modifyOpen(c) {
