    ...
    <div class="list-items">
      ...
      <ul class="list-wrap">
        ...
        >
          ...
          <span><input type="text" v-model="res.machineid" readonly/></span>
          <span><input type="text" v-model="res.paid_fee" readonly/></span>
  ...
  data() {
    return {
      listItem: [
        ...
        { title: "정산기ID", sortBy: false, standard: "machineid" },
        { title: "결제요금", sortBy: false, standard: "paid_fee" },
    ...
    socket_process(data) {
      if (data.status === "ok" && data.kind === "get_payment_list") {
        ...
        for (let i = 0; i < this.vehicle_obj_list.length; i++) {
          ...
          print.정산기ID = this.vehicle_obj_list[i].machineid
          print.결제요금 = this.vehicle_obj_list[i].paid_fee
          print.지불방법 = this.vehicle_obj_list[i].how_to_pay
        ...
        for (let i = 0; i < this.vehicle_obj_list.length; i++) {
          let temp = this.vehicle_obj_list[i]
          if (temp.parking_fee > 0) {
            total_parking_fee += temp.parking_fee
          }
          if (temp.parking_fee > 0) {
            total_paid_fee += temp.paid_fee
          }
        }
        let print = {}
        print.입차시각 = "합계"
        ...
        print.구분 = ""
        print.차량번호 = ""
        print.주차요금 = total_parking_fee
        print.정산기ID = ""
        print.결제요금 = total_paid_fee
        print.지불방법 = ""
        print.메모 = ""
        print.이미지 = ""
        this.print_vehicle_obj_list.push(print)
        ...
        obj1.in_time = "합계"
        obj1.out_time = ""
        obj1.parking_duration = ""
        obj1.location = ""
        obj1.registered = ""
        obj1.lp = ""
        obj1.parking_fee = total_parking_fee
        obj1.machineid = ""
        obj1.paid_fee = total_paid_fee
        obj1.how_to_pay = ""
        obj1.contents = ""
        obj1.imageList = ""
        this.vehicle_obj_list.push(obj1)
