      ...
      <ul class="list-wrap">
        ...
        >
          ...
          <span class="resident_fee"
            ><input type="text" v-model="res.total_resident_fee" readonly
          /></span>
          <span><input type="text" v-model="res.how_to_pay" readonly/></span>
          <span class="contents"
            ><input
              type="text"
  ...
  data() {
    return {
      listItem: [
        ...
        { title: "지불방법", sortBy: false, standard: "how_to_pay" },
        {
          title: "메모",
          sortBy: false,
          standard: "contents",
  ...
  methods: {
    ...
    changeTitle() {
      this.mainTitle.forEach(title => {
        title.subMenu.forEach(add => {
          if (add.add === "settlementRec") this.titles = add.title
        })
      })
    },
    modifyClose() {
    ...
    modifyOpen(c) {
      ...
      if (c.discounted_reduction_list) {
        for (let i = 0; i < c.discounted_list.length; i++) {
          let temp_obj = c.discounted_list[i]
          let current_obj = {}
          current_obj.discounted_reduction_name =
            temp_obj.discounted_reduction_name
          current_obj.discounted_reduction_hour =
            temp_obj.discounted_reduction_hour
          current_obj.discounted_reduction_value =
            temp_obj.discounted_reduction_value
          current_obj.discounted_reduction_unit =
            temp_obj.discounted_reduction_unit
          current_obj.location = temp_obj.location
          this.current.payment_list.push(current_obj)
        }
      }
    ...
    socket_process(data) {
      ...
      if (data.status === "ok" && data.kind === "get_residnet_list") {
        ...
        data.status === "ok" &&
        data.kind === "get_vehicle_obj_list_out"
      ) {
        ...
        for (let i = 0; i < vehicle_obj_list.length; i++) {
          ...
          if (obj.kind === "lpr" || obj.kind == "fee_calculation_result") {
            ...
            obj1.lp = obj.lp
            // if(obj.paid_fee){
            //   obj1.paid_fee = obj.paid_fee
            //   obj1.total_paid_fee += obj1.paid_fee
            // }
            if (obj.how_to_pay) {
            ...
            if (obj.obj_in) {
              ...
              if (obj2.discounted_list) {
                ...
                for (let j = 0; j < obj2.discounted_list.length; j++) {
                  ...
                  temp_obj.discounted_time = format_time1(
                    new Date(obj3.discounted_time)
                  )
                  ...
                  if (test) {
                    temp_obj.free_parking_time =
                      obj3.free_parking_time / (1000 * 60)
                  } else {
                    temp_obj.free_parking_time = "60"
                  }
                  temp_obj.discounted_duration =
                    obj3.discounted_duration / (1000 * 60)
              ...
              if (obj2.discounted_coupon_list) {
                console.log(
                  "discounted_coupon_list : ",
                  obj2.discounted_coupon_list
                )
                ...
                for (let k = 0; k < obj2.discounted_coupon_list.length; k++) {
                  ...
                  temp_obj.discounted_coupon_machine_id =
                    obj3.discounted_coupon_machine_id
                  temp_obj.discounted_coupon_location =
                    obj3.discounted_coupon_location
                }
              }

              if (obj2.discounted_reduction_list) {
                console.log(
                  "discounted_reduction_list : ",
                  obj2.discounted_reduction_list
                )
                obj1.discounted_reduction_list = []
                // obj1.discounted_reduction_list = obj2.discounted_reduction_list
                for (
                  let k = 0;
                  k < obj2.discounted_reduction_list.length;
                  k++
                ) {
                  let temp_obj = {}
                  ...
                  // temp_obj.discounted_coupon_time = 10
                  temp_obj.discounted_reduction_name =
                    obj3.discounted_reduction_name
                  temp_obj.discounted_reduction_hour =
                    obj3.discounted_reduction_hour
                  temp_obj.discounted_reduction_value =
                    obj3.discounted_reduction_value
                  temp_obj.discounted_reduction_unit =
                    obj3.discounted_reduction_unit
                  temp_obj.discounted_reduction_location =
                    obj3.discounted_reduction_location
                }
              }
              if (obj2.payment_list) {
                ...
                for (let k = 0; k < obj2.payment_list.length; k++) {
                  ...
                  temp_obj.time =
                    obj2.payment_list[k].paydate +
                    "_" +
                    obj2.payment_list[k].paytime
        ...
        for (let i = 0; i < this.vehicle_obj_list.length; i++) {
          ...
          print.결제요금 = this.vehicle_obj_list[i].total_paid_fee
          print.결제방법 = this.vehicle_obj_list[i].how_to_pay
          ...
          print.이미지 = this.vehicle_obj_list[i].imageList[0].imageName
    ...
    download: function() {
      ...
      XLSX.writeFile(wb, config.parking_name + "_정산기록_" + postfix + ".xlsx")
    }
  },
  created() {
