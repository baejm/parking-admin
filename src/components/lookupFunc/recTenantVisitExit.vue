    ...
    <div class="list-items">
      ...
      <ul class="list-wrap">
        <li v-for="(res, index) in vehicle_obj_list" :key="index">
          ...
          /></span>
          <span class="time"
            ><input type="text" v-model="res.discounted_time" readonly
          /></span>
          ...
          <span class="discounted_duration"
            ><input type="text" v-model="res.discounted_duration" readonly
          /></span>
          <span class="resident_fee"
            ><input type="text" v-model="res.resident_fee" readonly
          /></span>
          <span><input type="text" v-model="res.resident_id" readonly/></span>
          <span class="contents"
            ><input
              type="text"
    ...
    return {
      listItem: [
        ...
        { title: "차량번호", sortBy: false, standard: "lp" },
        {
          title: "할인시간",
          sortBy: false,
          standard: "discounted_duration",
          class: "discounted_duration"
        },
        {
          title: "부담금액",
          sortBy: false,
        ...
        { title: "차량사진", standard: "", class: "picture_vichle" }
      ],
      isLoading: false,
      resistRocord: [],
      start_time: new Date(),
  ...
  methods: {
    ...
    changeTitle() {
      this.mainTitle.forEach(title => {
        title.subMenu.forEach(add => {
          if (add.add === "recTenantVisitExit") this.titles = add.title
        })
      })
    },
    openImg(res) {
    ...
    socket_process(data) {
      ...
      if (data.status === "ok" && data.kind === "get_resident_list") {
        ...
        data.status === "ok" &&
        data.kind === "get_resident_discounted_list_admin_out"
      ) {
        ...
        if (this.searchResidentID === "" || this.searchResidentID === "전체") {
          for (let i = 0; i < vehicle_obj_list.length; i++) {
            ...
            console.log("obj.obj_in : ", obj.obj_in)
            if (obj.obj_in && obj.obj_in.discounted_list) {
              for (let j = 0; j < obj.obj_in.discounted_list.length; j++) {
                if (
                  obj.kind === "lpr" ||
                  ...
                  obj1.in_time = format_time1(
                    new Date(obj.obj_in.loop_event_time)
                  )
                  obj1.discounted_time = format_time2(
                    new Date(obj.obj_in.discounted_list[j].discounted_time)
                  )
                  ...
                  let test = isNaN(
                    obj.obj_in.discounted_list[j].free_parking_time
                  )
                  ...
                  if (test) {
                    ...
                    if (
                      ...
                      obj1.free_parking_time =
                        obj.obj_in.discounted_list[j].free_parking_time /
                        (1000 * 60)
                    }
                  }
                  obj1.discounted_duration =
                    obj1.free_parking_time +
                    ", " +
                    obj.obj_in.discounted_list[j].discounted_duration /
                      (1000 * 60)
                  obj1.resident_id = obj.obj_in.discounted_list[j].id
                  let test1 = isNaN(obj.obj_in.discounted_list[j].resident_fee)
                  console.log("test1 : ", test1)
                  if (test1) {
                    obj1.resident_fee = 0
                  } else {
                    obj1.resident_fee =
                      obj.obj_in.discounted_list[j].resident_fee
                  }
          ...
          for (let i = 0; i < vehicle_obj_list.length; i++) {
            ...
            for (let j = 0; j < obj.obj_in.discounted_list.length; j++) {
              if (obj.kind === "lpr" || obj.kind == "fee_calculation_result") {
                if (obj.obj_in && obj.obj_in.discounted_list) {
                  if (
                    obj.obj_in.discounted_list[j].id === this.searchResidentID
                  ) {
                    let obj1 = {}
                    obj1.out_time = format_time1(new Date(obj.loop_event_time))
                    obj1.in_time = format_time1(
                    ...
                    obj1.discounted_time = format_time1(
                      new Date(obj.obj_in.discounted_list[j].discounted_time)
                    )
                    obj1.loop_event_time = obj.loop_event_time
                    obj1.location = obj.location
                    ...
                    console.log(
                      "isNaN(obj.discounted_list[j].free_parking_time) : ",
                      isNaN(obj.discounted_list[j].free_parking_time)
                    )
                    if (test1) {
                      obj1.resident_fee =
    ...
    search() {
      ...
      var obj = {}
      obj.kind = "get_resident_discounted_list_admin_out"
    ...
    download: function() {
      ...
      XLSX.writeFile(
        wb,
        config.parking_name + "_입주사방문기록(출차)_" + postfix + ".xlsx"
      )
    }
  },
  created() {
